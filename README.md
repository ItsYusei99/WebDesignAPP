...
Created departments and orders tables via migrations.

Updated the users table with department_id (Foreign Key) and is_active (Boolean).

Configured Eloquent relationships (User belongs to Department).

Implemented Laravel Soft Deletes on the Order model to allow archiving.

## Controllers

HomeController: Added invoice search logic for the public view.

UserController: Implemented standard CRUD, department assignment, and logical user deactivation.

OrderController: Implemented CRUD, status updates (including evidence photo uploads for specific statuses), and archive/restore methods.

## Note
After pulling these changes, please run the following commands to update your local database and enable image storage:
